...
public class Datum {
   ...
   //This setter is controlling	if the day can be used
	public void setDen(int den) {
		if (this.getMesic() == 2) {
			if ((den > 28) || (den < 1)) {
				JOptionPane.showMessageDialog(null, "Tento mìsíc nemá tolik dnù!! Zadejte nové datum.", "WARNING - Neexistující datum", JOptionPane.WARNING_MESSAGE);
   ...
   //This setter is controlling	if the month can be used
	public void setMesic(int mesic) {
		if ((mesic > 12) || (mesic < 1)){
			JOptionPane.showMessageDialog(null, "Tento mìsíc neexistuje! Rok má 12 mìsícù.", "WARNING - Neexistující datum", JOptionPane.WARNING_MESSAGE);
			window.povoleno = false;
		} else {
			this.mesic = mesic;
		}
	...
	public void setRok(int rok) {
		if (rok < 2022) {
			JOptionPane.showMessageDialog(null, "Tento rok ji byl! Zadejte novı rok.", "WARNING - Neexistující datum", JOptionPane.WARNING_MESSAGE);
			window.povoleno = false;
		} else {
			this.rok = rok;
		}
