...
public class Zavodnik extends Osoba {
	...
	public void setVaha(double vaha) {
		if ((vaha > 150) || (vaha < 15)) {
			JOptionPane.showMessageDialog(null, "Závodník s touto váhou nemùže závodit! Maximální pøijatelná váha je 150kg a minimální 15kg.", "WARNING - Nepøijatelná váha", JOptionPane.WARNING_MESSAGE);
			...
			if (window.K.VahoveKategorie.getSelectedIndex() == window.K.VahoveKategorie.getItemCount()-1) {
				...
				JOptionPane.showMessageDialog(null, "Závodník s touto váhou nemùže závodit ve vámi vybrané vìkové kategorii!", "WARNING - Nepøijatelná váha", JOptionPane.WARNING_MESSAGE);
				window.povoleno = false;
			}
		} else {
			this.vaha = vaha;
	...
	public void KontrolaPaskuZavodnik() {
		for (int i = 0; i < TechnickeStupneZavodnici.length; i++) {
			if (this.getPasek() != TechnickeStupneZavodnici[i]) {
				System.out.println("S tímto páskem závodník nesmí závodit v mastogi!");
			} 
		}
	}
