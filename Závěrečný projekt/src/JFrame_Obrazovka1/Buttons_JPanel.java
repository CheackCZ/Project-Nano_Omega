...
public class Buttons_JPanel extends JPanel {
	...
	public Buttons_JPanel(Window window, Window2 zapas) {
		...
		//Button that sends you to another screen	
				...
				presunNaZapas = new JButton("Pøesunout na zápas");
			...
			//Button to set everything
				...
				potvrzeniUdaju.setToolTipText("Potvrzení zadaných údajù");
				potvrzeniUdaju.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (kontrolaKategorie.KontrolaVyplneni() == false) {
							JOptionPane.showMessageDialog(null, "Chybí údaj/e v kategoriích zápasu!", "ERROR", JOptionPane.ERROR_MESSAGE);
						} else if (kontrolaUdajuCerveny.KontrolaVyplneni() == false) {
							JOptionPane.showMessageDialog(null, "Chybí údaj/e u èerveného závodníka!", "ERROR", JOptionPane.ERROR_MESSAGE);
						} else if (kontrolaUdajuModry.KontrolaVyplneni() == false) {
							JOptionPane.showMessageDialog(null, "Chybí údaj/e u modrého závodníka!", "ERROR", JOptionPane.ERROR_MESSAGE);
						} else if (kontrolaUdajuRozhodci.KontrolaVyplneni() == false) {
							JOptionPane.showMessageDialog(null, "Chybí údaj/e u rozhodèího!", "ERROR", JOptionPane.ERROR_MESSAGE);
						} else if (kontrolaUdajuZapas.KontrolaVyplneni() == false) {
							JOptionPane.showMessageDialog(null, "Chybí údaj/e v údajích o zápase!", "ERROR", JOptionPane.ERROR_MESSAGE);
						} else if (window.povoleno == false) {
							presunNaZapas.setEnabled(false);
						} else if (window.datumPovoleno == false) {
							JOptionPane.showMessageDialog(null, "Licence již není platná! Zadejte jiné datum licence.", "WARNING - Neplatná licence rozhodèího", JOptionPane.WARNING_MESSAGE);
						} else {
			...
			//Button to edit everything
				...
				upraveniUdaju.setToolTipText("Upravení údajù");
			...
			//Button to refresh everything	
				...
				refreshUdaju.setToolTipText("Refresh údajù");
